...
def run_walker(listener2, tree, walker):
    print(walker.__dir__())
...
def main(filename):
    ...
    output_walker = ""
    # output_walker = subprocess.run([sys.executable, "-c", "run_walker({listener2}, {tree}, {walker})"])

    f = io.StringIO()
    with redirect_stdout(f):
        run_walker(listener2, tree, walker)
    output_walker = f.getvalue()

    output_scopes = pretty_printing(listener2.memory['scopes'])

    return output_walker, output_scopes
...
def run_file():
    ...
    output, output_scopes = main(filename)
    print(output_scopes)
